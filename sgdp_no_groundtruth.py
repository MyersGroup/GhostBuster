...
def fixed_parameters(base_path, poplabels, unique_groups, num_trees, target_seq_, subset_ids, chrs):
    eps = 1e-20
    # tree = ts_list[0].first()
    # num_samples = len(list(tree.samples()))
    coal_count = np.zeros((len(unique_groups), len(epoch_intervals_pow)-1, num_trees))
    ...
    for chr in chrs:
        ...
        for tid in tqdm(range(ts.num_trees - 1)): #len(list(ts.trees()))
            ...
            # for m in range(len(poplabels)):
            #     lineage_content[2*m:2*m + 2, group_id[poplabels.POP.loc[m]]] = 1
...
def main(sample_id, plot = False, gamma_arr = None):
    start_time = time.time()
    num_clusters = 3
    poplabels = pd.read_csv('/well/myers/users/ooz218/workspace/MixedAncestryCoalescenceRates/SGDP_whole_genome/SGDP_relate/data/SGDP_afr_eurasians_v2.poplabels', sep =' ')
    subset_ids = (2*poplabels.index).tolist() + (2*poplabels.index + 1).tolist()
    unique_groups = np.unique(poplabels.GROUP)#np.unique(poplabels.POP)#
    print(unique_groups)
    ...
    num, denom, proportion_of_coalescing_all, epoch_index_all = fixed_parameters(base_path, poplabels, unique_groups, num_trees, sample_id, subset_ids, chrs = chrs)

    log_likelihood_arr = []
    start_time_em = time.time()
    print("Starting the EM..")
    prev_gamma = None ## start with None
    for epoch in range(50):  ## max-iters = 40
        gamma_arr = np.zeros((len(own_membership), len(unique_groups), len(epoch_intervals) - 1))        
        ## M-step
        for j in range(len(own_membership)):
            if epoch == 0:
                n = compute_gamma_num(own_membership[j], None, proportion_of_coalescing_all, epoch_index_all, len(unique_groups))
            else:
                n = compute_gamma_num(own_membership[j],prev_gamma[j] , proportion_of_coalescing_all, epoch_index_all, len(unique_groups)) #compute_gamma_num(own_membership[j], prev_gamma[j], proportion_of_coalescing_all, epoch_index_all, len(unique_groups))
        ...
        # own_membership_update = np.nan_to_num(own_membership_update, nan = 0.5)
        for j in range(len(own_membership)):
        ...
        for i in range(gamma_arr.shape[0]):
            ...
            for j in range(gamma_arr.shape[1]):
                ax = plt.plot(gamma_arr[i][j], marker = 'o', color = colors[j])        
            # plt.legend(['Africa', 'America', 'CentralAsia', 'EastAsia', 'Oceania', 'SouthAsia', 'WestEurasia'], fontsize = 14)
            ax = plt.legend(unique_groups, fontsize = 14)
            ax = plt.xlabel('Epochs', fontsize=14)
            ax = plt.ylabel('Gamma', fontsize = 14)
            plt.show()
            plt.savefig('mbuti_gamma_' + str(i) + '_iter_' + str(epoch) + '.png')
            plt.close()  
        print("log-likelihood = " + str(log_likelihood_arr[-1]))

        # Early-stopping
        if epoch > 25: ##min-iters = 10
    ...
    return gamma_arr, log_likelihood_arr, own_membership, unique_groups

acc = 0
sample_id = 21 ## S_Mbuti-3
# sample_id = 253 ## S_Maori-1
# sample_id = [399, 173]  ##S_Mbuti-3 (21)/S_English-1(399) and S_Han-1
gamma_arr, log_likelihood_arr, own_membership, unique_groups = main(sample_id, plot=False, gamma_arr =  None)
colors_afr = cm.Blues(np.linspace(0, 1, 6))
colors_non_afr = ['r','y','g','c','m','k']
...
for i in range(gamma_arr.shape[0]):
    ...
    plt.clf()
    for j in range(gamma_arr.shape[1]):
        if unique_groups[j] == 'Mozabite' or unique_groups[j] == 'Saharawi' or unique_groups[j] == 'Somali':
            color = colors_afr[1]
            labels.append('Afroasiatic')
        elif unique_groups[j] == 'BantuHerero' or unique_groups[j] == 'BantuTswana' or unique_groups[j] == 'BantuKenya' or unique_groups[j] == 'Luhya':
            color = colors_afr[2]
            labels.append('Niger-Kordofanian-Bantu')
        elif unique_groups[j] == 'Biaka' or unique_groups[j] == 'Gambian' or unique_groups[j] == 'Yoruba' or unique_groups[j] == 'Esan' or unique_groups[j] == 'Mandenka' or unique_groups[j] == 'Mende':
            color = colors_afr[3]
            labels.append('Niger-Kordofanian')
        elif unique_groups[j] == 'Mbuti' or unique_groups[j] == 'Luo' or unique_groups[j] == 'Masai' or unique_groups[j] == 'Dinka':
            color = colors_afr[4]
            labels.append('Nilo-Saharan')
        elif unique_groups[j] == 'Ju_hoan_North' or unique_groups[j] == 'Khomani_San':
            color = colors_afr[5]
            labels.append('Khoesan')
        else:
            color = colors_non_afr[count_out_of_afr]
            labels.append(unique_groups[j])
            count_out_of_afr += 1
        plt.plot(gamma_arr[i][j], marker = 'o', color = color, linewidth=4.0, label = labels[j])       
    ax = fig.gca() 
    for k, p in enumerate(ax.get_lines()):
        if p.get_label() in labels[:k]:
            idx = labels.index(p.get_label())
            p.set_c(ax.get_lines()[idx].get_c())
            p.set_label('_' + p.get_label()) 
    plt.legend(fontsize = 28, loc='upper right') #(unique_groups, fontsize = 28, loc='upper right')
    plt.xlabel('Epochs', fontsize=28)
    plt.ylabel('Gamma', fontsize = 28)
    plt.ylim(0, 2.5e-4)
    plt.show()
    plt.savefig('mbuti_gamma_' + str(i) + '.png')
    plt.close()  

plt.clf()
plt.figure(figsize=(40,4))
sns.heatmap(own_membership)
plt.savefig('own_membership_mbuti.png')
plt.close()
plt.clf()      
plt.plot(log_likelihood_arr)
plt.savefig('sgdp_log_likelihood_mbuti.png')
plt.close()
