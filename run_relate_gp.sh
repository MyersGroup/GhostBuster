...
ind=51
path="../sims/stdpopsim_ancient_small/relate_trees_force_50/"
...
python bayesopt.py --wandb_offline --start_time ${start_time} --end_time ${end_time} --path ${path} --trees ${trees} --chrs ${chrs} --tree_stats_file_name ${out}_${k}_bayes_tree_stats_${ind}_${window_size}_True_${chrs}_${masking_thresh}.pkl  --fixed_params_file_name ${out}_${k}_bayes_fixed_params_${ind}_${window_size}_True_${chrs}_${masking_thresh}.pkl  --sample_id ${ind} --ignore_first_epoch ${ignore_first_epoch} -k ${k} --num_epochs 4 --output ${out}_${k}_overall_membership_bayes.npy --bayes_steps 100


## Running EM

echo "3. Running the EM"
python em_true_ancient_sim_subsampled.py --mode real --chr ${chrs} --relate_trees True --masking_thresh ${masking_thresh} --plot_intermediate_gammas True --window_size ${window_size} --sample_id ${ind} --init_at_truth 0 --path ${path} --trees ${trees} --output ${out}_${k} -k ${k} -i ${i} -start_time ${start_time} -end_time ${end_time} --num_epochs ${num_epochs} -ignore_first_epoch ${ignore_first_epoch} --rec ${recomb_map} --verbose True --check_muts_target ${check_muts_target} --load_membership ${out}_${k}_overall_membership_bayes.npy 
