...
def compute_tree_stats(ts_list, chrs, window_size):
    ...
    for chr in chrs:
        ...
        #relate_quality_output = pd.read_csv(path + '/stdpopsim_homsap/Han_only/relate_homsap_ne_chr' + str(chr) + '.qual', sep = ' ')
        #relate_quality_output = pd.read_csv(path + '/stdpopsim_homsap/moderns_only/relate_homsap_ne_chr' + str(chr) + '.qual', sep = ' ')
        #relate_quality_output = pd.read_csv(path + '/stdpopsim_homsap/Mbuti_Han/relate_homsap_ne_chr' + str(chr) + '.qual', sep = ' ')
        relate_quality_output = pd.read_csv(path + 'SGDP_archaic_v1_EAS_chr' + str(chr) + '.qual', sep = ' ')
        ts = ts_list[count]
        count += 1
        tree = ts.first()
        # prev_interval = 0
...
def main(args, plot=False, gamma_arr=None):
    ...
    #membership = [(0,50,0,0), (50,52,1,320), (52,102,2,0), (102,104,3,300), (104,106,4,960), (106,156,5,0), (156,158,6,1800), (158, 160, 7,2000)]
    #membership = [(0,2,0,2607),(2,4,1,1964),(4,6,2,3928),(6,102,3,0)]
    membership = [(0,6,0,1964),(6,102,1,0)]
    #membership = [(0,6,0,1964),(6,12,1,0)]
    #membership = [(0,6,0,1964),(6,14,1,0)]
    #membership  = [(0,6,0,0), (6,12,1,0), (12,14,2,0)]

    #membership = [(0,50,0,0), (50,100,1,0)]
    #membership = [(0,50,0,0), (50,100,1,0), (100,150,2,0)]
    #membership = [(0,150,0,0)]
    #membership = [
    #     (0, 50, 0, 0),
    #     (50, 52, 1, 2000),
    ...
    for chr in chrs:
        ...
        if args.relate_trees:
            ...
            #ts = tskit.load(path + '/stdpopsim_homsap/Han_only/relate_homsap_ne_chr'+ str(chr) + '.trees')  ## relate trees
            #ts = tskit.load(path + '/stdpopsim_homsap/moderns_only/relate_homsap_ne_chr'+ str(chr) + '.trees')  ## relate trees
            ts = tskit.load(path + "/SGDP_archaic_v1_EAS_chr" + str(chr)+ ".trees")
            #ts = tskit.load(path + '/stdpopsim_homsap/Mbuti_Han/relate_homsap_ne_chr'+ str(chr) + '.trees')  ## relate trees
            #ts = tskit.load(
            #    path
            ...
            #    + ".trees"
            #)  ## relate trees
            #ts = ts.simplify(chain(range(50),range(100,150)))
        else:
            ts = tskit.load(
                path
                + "/stdpopsim_homsap/true_trees/stdpopsim_homsap_mod_chr"
                + str(chr)
                + ".trees"
            )  ## true trees
            #ts = ts.simplify(list(range(0,50)) + list(range(106,156)))
            ts = ts.simplify(list(range(106,156)))
        ts_list.append(ts)
    ...
    for epoch in range(1000):  ## max-iters = 40
        gamma_arr = np.zeros(
