...
#poplabs <- c("CentralAsiaSiberia", "WestEurasia")
# poplabs <- c( "Mozabite", "Saharwi", "Yoruba")
# poplabs <- c("Hazara", "Mongola", "Pathan" )
# poplabs <- c("Irula","NA", "Iranian")
poplabs <- c("AltaiNeandertal", "Chagyrskaya", "Denisova", "Han", "Mbuti", "Vindija")
print(poplabs)
#
...
read.coal <- function(filename){
	...
	print(coal)
        #coal <- subset(coal, group2 %in% c("Africa", "WestEurasia","CentralAsiaSiberia" ))
        #print(coal)
...
coal <- rbind(coal, cbind(read.coal(paste0(filename,"_",sam,".coal")), iter = paste0("Iteration 200")))

coal$group1 <- as.numeric(as.factor(coal$group1))

coal$haploid.coalescence.rate[which(coal$haploid.coalescence.rate == 0 & coal$epoch.start > 5e5/28)] <- coal$haploid.coalescence.rate[which(coal$haploid.coalescence.rate == 0 & coal$epoch.start > 5e5/28)-1]
...
p1 <- ggplot(coal) + geom_step(aes(x = 28*epoch.start, y = 0.5/mean, colour = group1), lwd = 1.1) +
				...
				ggthemes::theme_few() +
				scale_x_continuous(limit = c(5e3,5e6), trans = "log10") +
...
foo <- as.matrix(read.table(paste0(filename, "_" ,sam,".logl")))
...
foo <- read.table(paste0(filename,"_", sam,".tau"))
...
#
p <- plot_grid(p2,p3,ncol = 1, align = "hv", axis = "lr", labels = c("b", "c", "d", "e"), label_size = 17)
p <- plot_grid(p1,p, ncol = 2, rel_widths = c(2.0,1), labels = c("a", ""), label_size = 17)

ggsave(p, file = paste0(filename, "_", sam,".pdf"), width = 14.5, height = 9.5)
