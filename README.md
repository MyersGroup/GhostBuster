...
## About the code
* `<sgdp_no_groundtruth.py>` to generate coalescence rate plots on SGDP samples - for example the understanding the history for Mbuti samples
...
* `<em_true_ghost.py>` the same simulation but running with ghost groups or proxy groups
